...
public class CommandFlightFactory extends AbstractCommandEntityFactory<Flight> {
	...
	public Flight create(Map<String, String[]> map) throws EntityFactoryException {
		...
		departureTime = parseStringToDate(map.get(StringConstant.FLIGHT_DEPARTURE_TIME_KEY.getValue())[ZERO_INDEX]);
		landingTime = parseStringToDate(map.get(StringConstant.FLIGHT_LANDING_TIME_KEY.getValue())[ZERO_INDEX]);
		...
		try {
			ServiceFactory.INSTANCE.getAircraftService().getAircraftByID(flightAircraftID)
					.ifPresent(flight::setAircraft);
			ServiceFactory.INSTANCE.getAirportService().getAirportByID(flightDepartureAirportID)
					.ifPresent(flight::setDepartureAirport);
			ServiceFactory.INSTANCE.getAirportService().getAirportByID(flightDestinationAirportID)
					.ifPresent(flight::setDestinationAirport);
